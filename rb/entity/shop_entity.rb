...
class ShopEntity
  ...
  def make
    opts = @_entopts.dup
    ShopEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FortniteHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Shop, Hash] the current Shop data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FortniteHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Shop fields)
  def match_get
  ...
  # Load a single Shop.
  #
  # @param reqmatch [ShopLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Shop.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Shop, Hash] the loaded Shop; raises FortniteError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FortniteHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
