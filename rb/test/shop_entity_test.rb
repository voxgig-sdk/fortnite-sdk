...
class ShopEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = shop_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "shop." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FORTNITE_TEST_SHOP_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    shop_ref01_data = nil
    if shop_ref01_data_raw.length > 0
...
def shop_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "shop", "ShopTestData.json")
  ...
  entid_env_raw = ENV["FORTNITE_TEST_SHOP_ENTID"]
  ...
  if env["FORTNITE_TEST_LIVE"] == "TRUE"
    ...
    client = FortniteSDK.new(Helpers.to_map(merged_opts))
  end
