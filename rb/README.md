...
The Ruby SDK for the Fortnite API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Cosmetic` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = FortniteSDK.test(testopts, sdkopts)
```
...
| `Shop` | `(data) -> ShopEntity` | Create a Shop entity instance. |
...
raise a `FortniteError` (a `StandardError` subclass), so wrap
...
The main module (`Fortnite_sdk`) exports the SDK class
