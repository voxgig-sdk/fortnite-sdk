...
module FortniteUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = FortniteResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FortniteResponse.new(fetched)
    else
