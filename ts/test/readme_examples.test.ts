...
//     (\`new FortniteSDK(...)\` / \`FortniteSDK.test(...)\` become
//     \`FortniteSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"cosmetic":{"test01":{"id":"test01"}},"shop":{"test01":{"id":"test01"}},"statistic":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FortniteSDK(...)\` and any
// \`FortniteSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FortniteSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FortniteSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
