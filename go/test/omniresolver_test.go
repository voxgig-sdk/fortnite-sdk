...
import (
	...
	omni "github.com/voxgig-sdk/fortnite-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FortniteError); is {
				out := map[string]any{
					"name":    "FortniteError",
...
func omniCtx(arg any, client *sdk.FortniteSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FortniteSDK); is {
			client = live
