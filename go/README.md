...
## Install
```bash
go get github.com/voxgig-sdk/fortnite-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/fortnite-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/fortnite-sdk/go=../fortnite-sdk/go
```
...
func main() {
    ...
    // List cosmetic records — the value is the array of records itself.
    cosmetics, err := client.Cosmetic(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range cosmetics.([]any) {
        fmt.Println(item)
    }
}
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *FortniteSDK
```
...
| `Cosmetic` | `(data map[string]any) FortniteEntity` | Create a Cosmetic entity instance. |
| `Shop` | `(data map[string]any) FortniteEntity` | Create a Shop entity instance. |
| `Statistic` | `(data map[string]any) FortniteEntity` | Create a Statistic entity instance. |

### Entity interface (FortniteEntity)

All entities implement the `FortniteEntity` interface.
...
Create an instance: `shop := client.Shop(nil)`
...
fmt.Println(shop) // the loaded record
```
...
```
github.com/voxgig-sdk/fortnite-sdk/go/
├── fortnite.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/fortnite-sdk/go`) re-exports everything needed
