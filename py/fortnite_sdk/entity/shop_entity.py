...
from fortnite_sdk.fortnite_types import (
    Shop,
    ShopLoadMatch,
)
...
class ShopEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ShopEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Shop:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Shop().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
