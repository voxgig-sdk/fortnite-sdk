...
The Python SDK for the Fortnite API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Cosmetic(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Fortnite(None).load(
    {"id": "test01"}, None
)
