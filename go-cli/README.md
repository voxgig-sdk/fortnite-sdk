...
boru-driven command-line client **and** interactive REPL for the Fortnite
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fortnite-cli)
make build
...
./fortnite-cli list cosmetic
./fortnite-cli list shop

# 5. Override the API base URL for a single call
FORTNITE_BASE=https://api.example.com ./fortnite-cli list cosmetic

# 6. No arguments -> interactive REPL
./fortnite-cli
fortnite> list cosmetic
fortnite> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fortnite-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/fortnite-cli list cosmetic
   ```
...
Run with no arguments to open a REPL (prompt `fortnite>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `cosmetic`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/fortnite-cli`. |
...
(`./fortnite-cli <expr>`) and interactively (the REPL), and expressions compose the
