// Command Fortnite-cli is a CLI + REPL for the Fortnite SDK,
...
const prompt = "fortnite"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "cosmetic shop statistic"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FORTNITE_APIKEY (api key), FORTNITE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FORTNITE_APIKEY carries the API key and
	// FORTNITE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FORTNITE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FORTNITE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFortniteSDK(opts)
