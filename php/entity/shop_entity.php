...
class ShopEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ShopEntity($this->_client, $opts);
    }

    /**
     * @param Shop|array $args Shop data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = FortniteHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Shop|array The current Shop data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Shop fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = FortniteHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Shop fields).
     */
     ...
     * Load a single Shop.
     *
     * @param ShopLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ShopLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Shop|array The loaded Shop as an assoc-array at the
     *   SDK boundary; throws FortniteError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = FortniteHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
