...
class FtCtrl extends FortniteControl
{
...
class FtClient
{
    ...
    private FortniteUtility $utility;

    public function __construct(FortniteUtility $utility, array $options)
    {
    ...
    public function get_utility(): FortniteUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FortniteContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FortniteContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new FortniteUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (FortniteContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FortniteConfig::make_feature($name);
    ...
    private static function buildUrl(FortniteSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FortniteError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FortniteResponse($fetched);
        }
    ...
    private function fail(FortniteContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FortniteContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
