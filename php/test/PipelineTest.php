...
{
    private static function utility(): FortniteUtility
    {
        return new FortniteUtility();
    }
    ...
    private static function ctx(array $over = []): FortniteContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FortniteContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FortniteOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FortniteError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FortniteOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FortniteMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FortniteOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FortniteMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FortniteOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FortniteMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FortniteSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FortniteMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FortniteResponse([]),
            'result' => new FortniteResult([]),
        ]);
        [, $err] = FortniteMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FortniteSpec([]),
            'response' => null,
            'result' => new FortniteResult([]),
        ]);
        [, $err] = FortniteMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FortniteSpec(['step' => 's']),
            'response' => new FortniteResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FortniteResult([]),
        ]);
        [, $err] = FortniteMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FortniteSpec(['step' => 's']),
            'response' => new FortniteResponse(self::resp(200, ['v' => 1])),
            'result' => new FortniteResult([]),
        ]);
        [, $err] = FortniteMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FortniteSpec(['step' => 's']),
            'response' => new FortniteResponse(self::resp(200, ['v' => 2])),
            'result' => new FortniteResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FortniteMakeResponse::call($ctx);
    ...
    {
        $preset = new FortniteResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FortniteSpec([]),
            'response' => new FortniteResponse([]),
            'result' => new FortniteResult([]),
        ...
        [$got, $err] = FortniteMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FortniteResult([])]);
        [, $err] = FortniteMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FortniteSpec([]), 'result' => null]);
        [, $err] = FortniteMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FortniteSpec(['step' => 's']),
            'result' => new FortniteResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FortniteOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FortniteMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FortniteSpec(['step' => 's']),
            'result' => new FortniteResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FortniteOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FortniteMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FortniteResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FortniteSpec([]),
            'result' => new FortniteResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FortniteMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FortniteMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FortniteError('boom', 'boom');
        $utility->fetcher = function (FortniteContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FortniteSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FortniteMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FortniteContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FortniteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FortniteResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FortniteContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FortniteSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FortniteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FortniteResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FortniteSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FortniteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FortniteMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FortniteMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FortniteResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FortniteDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FortniteResult(['ok' => false])]);
        $this->expectException(FortniteError::class);
        FortniteDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FortniteResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FortniteMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FortniteMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FortniteBaseFeature();
        $b = new FortniteBaseFeature();
        FortniteFeatureAdd::call($ctx, $a);
        FortniteFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FortniteBaseFeature
    {
        $f = new FortniteBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FortniteFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FortniteContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FortniteSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FortnitePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FortniteResponse(['headers' => null]),
            'result' => new FortniteResult([]),
        ]);
        FortniteResultHeaders::call($ctx);
