...
class FortniteNetsimFeature extends FortniteBaseFeature
{
    ...
    private function _simulate(FortniteContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(FortniteContext $ctx, array $applied): void
    {
        $client = $this->client;
