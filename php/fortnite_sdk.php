...
class FortniteSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = FortniteHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = FortniteHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!FortniteFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, FortniteFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return FortniteUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = FortniteHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = FortniteHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = FortniteHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new FortniteError($op . "_allow",
                "FortniteSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = FortniteHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new FortniteError("graphql_error",
                "FortniteSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Shop($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_shop === null) {
                $this->_shop = new ShopEntity($this, null);
            }
            return $this->_shop;
        }
        return new ShopEntity($this, $data);
    }
