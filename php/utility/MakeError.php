...
class FortniteMakeError
{
    public static function call(?FortniteContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FortniteContext([], null);
        }
        $op = $ctx->op ?? new FortniteOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FortniteResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FortniteError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FortniteError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof FortniteError) {
