...
function ShopEntity.new(client, entopts)
  entopts = entopts or {}
...
function ShopEntity:get_name()
  return self._name
end


function ShopEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ShopEntity.new(self._client, opts)
end
...
function ShopEntity:mark_deleted()
  self._deleted = true
end


function ShopEntity:deleted()
  return true == self._deleted
end


function ShopEntity:data_set(args)
  if args ~= nil then
...
function ShopEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ShopEntity:match_set(args)
  if args ~= nil then
...
function ShopEntity:stream(action, args, callopts)
...
---@param reqmatch ShopLoadMatch
---@param ctrl? table
---@return Shop
---@return string? err
function ShopEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ShopEntity:_run_op(ctx, post_done)
  local utility = self._utility
