...
The Lua SDK for the Fortnite API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Cosmetic(nil):list(nil, nil)
...
local result, err = client:Fortnite(nil):load(
  { id = "test01" }, nil
)
