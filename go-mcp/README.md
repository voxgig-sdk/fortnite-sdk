...
[MCP](https://modelcontextprotocol.io) server exposing the Fortnite SDK as
two agent tools — `fortnite_list` and `fortnite_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fortnite-mcp)
make build

# 2. Provide credentials via the environment
export FORTNITE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user fortnite \
  -- /absolute/path/to/fortnite-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./fortnite-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// fortnite_list: first page of records
{ "entity": "cosmetic" }
{ "entity": "cosmetic", "query": { } }

// fortnite_load: one record by id
{ "entity": "shop", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fortnite-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user fortnite \
     -- "$PWD"/dist/*/fortnite-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `fortnite_list` and `fortnite_load` tools now appear
   in new sessions. Ask the agent to *"list cosmetic using fortnite"*
   and it calls `fortnite_list` with `{"entity":"cosmetic"}`.
...
```sh
export FORTNITE_APIKEY=sk_live_xxx            # API key
export FORTNITE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `fortnite_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `fortnite_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FORTNITE_APIKEY` | API key sent with every request. |
| `FORTNITE_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

cosmetic | shop | statistic

### Smoke test via HTTP (raw JSON-RPC)

```sh
./fortnite-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"fortnite_load","arguments":{"entity":"shop","query":{"id":1}}}}'
```
